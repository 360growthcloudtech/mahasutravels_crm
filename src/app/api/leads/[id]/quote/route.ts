...
async function assertLeadAccess(leadId: string, sessionSub: string, role: string) {
  const lead = await findLeadById(leadId);
  if (!lead) return { error: NextResponse.json({ error: "Lead not found" }, { status: 404 }) };
  if (role === "Employee" && lead.assigned_to !== sessionSub) {
...
export async function GET(
  ...
  const denied = forbidUnlessAnyPermission(session, ["leads.view", "leads.quote"]);
  if (denied) return denied;

  const { id } = await context.params;
  const access = await assertLeadAccess(id, session.sub, session.role);
  if (access.error) return access.error;

  const quote = await findLatestLeadQuote(id);
...
export async function PUT(
  ...
  const quote = await upsertLeadQuoteDraft(id, input);
  return NextResponse.json({ quote });
}
